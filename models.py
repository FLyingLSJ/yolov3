...
def create_modules(module_defs, img_size, arc):
    """
    通过解析 cfg 后得到的结果进行创建模型
    返回
    module_list:PyTorch 模型结果
    routs（多尺度融合的参数） 一个列表
    """
    # Constructs module list of layer blocks from module configuration in module_defs

    hyperparams = module_defs.pop(0) # [Net] 头，包含基本的参数
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            modules.add_module('cbam_module', CBAM(output_filters[-1], reduction_ratio=int(mdef['reduction_ratio']), kernel_size=int(mdef['kernel_size'])))
...
class Darknet(nn.Module):
    ...
    def forward(self, x, var=None):
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            # 卷积层，上采样，池化层，se（注意力机制模块）只需要经过即可
                x = module(x)
        ...
        if self.training:  # train
            ...
            # 3*(class+5)
            return yolo_out
        elif ONNX_EXPORT:  # export # 这个是对应的onnx导出的内容
...
if __name__=="__main__":
    module_list, routs = create_modules(parse_model_cfg("./cfg/yolov3-tiny-20cls.cfg"), 416, arc="default")
